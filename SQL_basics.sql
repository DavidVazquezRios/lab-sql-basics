...
*/
SELECT card.type, count(card.type)
FROM card
GROUP BY card.type
ORDER BY count(card.type) desc;
/*
Expected result:
...
*/
SELECT 
    loan.date, duration, COUNT(loan_id)
FROM
    loan
WHERE
    loan.date BETWEEN 971200 AND 971232
GROUP BY duration , loan.date
ORDER BY loan.date ASC;
...
*/
SELECT 
    account_id, trans.type, SUM(amount) as `total_amount`
FROM
    trans
WHERE
    account_id = 396
